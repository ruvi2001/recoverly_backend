...
@public_router.get("/arrs/latest")
async def latest_arrs_session(
    db: AsyncSession = Depends(get_db),
):
    q = (
        select(ARRSSession)
        .where(ARRSSession.user_id == "test_user_003")
        .order_by(desc(ARRSSession.created_at), desc(ARRSSession.session_id))
        .limit(1)
    )

    session = (await db.execute(q)).scalar_one_or_none()
    if not session:
        raise HTTPException(status_code=404, detail="No ARRS session found")
    ...
    return {
        ...
        "predicted_category": session.predicted_category,
        "predicted_family": session.predicted_category,
        "confidence": session.confidence,
        "answers": session.answers_json,
        "recommendation": session.recommendation_json,
