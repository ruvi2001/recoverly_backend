...
async def run_assessment(db: AsyncSession, patient_id: str, assessment_date: date, features: Dict[str, int]) -> Dict:
    assessment = Assessment(patient_id=patient_id, assessment_date=assessment_date, **features)
    db.add(assessment)
    await db.flush()
    ...
    return {
        ...
        "prediction_id": prediction.prediction_id,
        **{k: result[k] for k in [
            "predicted_label", "predicted_risk_percent",
            "category", "emoji", "total_score", "model_version", "xai"
        ]},
    }

async def get_patient_history(db: AsyncSession, patient_id: str):
    result = await db.execute(
        ...
        .options(
            ...
            selectinload(Assessment.followup),
        )
